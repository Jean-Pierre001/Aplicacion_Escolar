...
<div class="flex-1 md:ml-64 transition-all duration-300">
  ...
  <main class="pt-20 p-4 md:p-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-3 md:gap-0">
      ...
      <a href="javascript:void(0)" onclick="openModal('addClassroomModal')" class="inline-flex items-center bg-blue-600 text-white px-5 py-2 rounded-lg shadow hover:bg-blue-700 transition w-full md:w-auto justify-center">
    ...
    <div class="overflow-x-auto rounded-lg shadow-lg border border-gray-200">
      <table class="min-w-full border-collapse" id="classroomsTable">
          ...
          try {
              ...
              if ($classrooms) {
                  foreach ($classrooms as $i => $classroom) {
                      ...
                      echo "<td class='px-4 md:px-6 py-4 border-r border-gray-300'>{$classroom['name']}</td>";
                      echo "<td class='px-4 md:px-6 py-4 border-r border-gray-300'>{$classroom['description']}</td>";
                      echo "<td class='px-4 md:px-6 py-4 flex flex-wrap gap-2'>";
                      echo "<a href='javascript:void(0)' onclick='openEditModalClassroom(".json_encode($classroom).")' class='text-yellow-500 hover:text-yellow-700 bg-yellow-100 px-3 py-1 rounded flex items-center justify-center w-24'>
                              <i class='fa-solid fa-pen mr-1'></i>Editar
                            </a>";
                      echo "<a href='classrooms_back/delete_classroom.php?id={$classroom['classroom_id']}' class='text-red-600 hover:text-red-800 bg-red-100 px-3 py-1 rounded flex items-center justify-center w-24' onclick=\"return confirm('¿Estás seguro de eliminar esta aula?')\">
                  ...
                  echo "<tr><td colspan='3' class='px-4 md:px-6 py-4 text-center text-gray-500'>No hay aulas registradas</td></tr>";
              }
